/*
 * Modified version of file below. Added Avis public call-point, fixed 
...
Key avis_sha1 (Key input)
{
  ...
  key.data = emalloc (SHA1_DIGEST_SIZE);
  key.length = SHA1_DIGEST_SIZE;
  
  SHA1_Init (&context);
  
  SHA1_Update (&context, input.data, input.length);
  
  SHA1_Final (&context, key.data);
